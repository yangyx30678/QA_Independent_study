...
class Squad(Dataset):
    def __init__(self, ver, split) -> None:
        super().__init__()
        with open(f'{file_dict}squad{ver}_{split}.json', 'r') as f:
...
if __name__ == '__main__':
    ...
    squad1 = Squad(ver=1, split='train')
    # cmrc = CMRC(split='test')
    c = s.mean([len(each['c']) for each in squad1])
    q = s.mean([len(each['q']) for each in squad1])
    print(q, c)
    # for each in cmrc:
